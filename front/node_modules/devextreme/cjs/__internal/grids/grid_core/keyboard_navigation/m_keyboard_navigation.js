...
class KeyboardNavigationController extends _m_keyboard_navigation_core.KeyboardNavigationController {
    ...
    _updateFocus(isRenderView) {
        ...
        this._updateFocusTimeout = setTimeout((() => {
            ...
            if (!this.getMasterDetailCell($cell) || this._isRowEditMode()) {
                ...
                if ($cell && (0, _m_keyboard_navigation_utils.isElementDefined)($cell)) {
    ...
    adjustCellOnVerticalNav($cell, position, keyCode) {
        ...
        if (!isVerticalNav || !(0, _m_keyboard_navigation_utils.isElementDefined)($cell) || this._isCellValid($cell)) {
            return $cell
        }
    ...
    resolveInvalidCell($cell, position, keyCode) {
        if (!(0, _m_keyboard_navigation_utils.isElementDefined)($cell) || this._isCellValid($cell)) {
            return $cell
        }
        const isRowFocus = this.isRowFocusType();
        const includeCommandCells = isRowFocus || "next" === keyCode || "previous" === keyCode;
        const isBoundaryCell = "previous" === keyCode ? this._isFirstValidCell(position) : this._isLastValidCell(position);
